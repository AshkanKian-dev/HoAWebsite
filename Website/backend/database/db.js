...
// Password: HoA_Admin_2024!Secure#Key$987
...
const dbHelpers = {
  ...
  createUser: (userData) => {
    ...
    const userId = uuidv4();
    const stmt = db.prepare(`
      INSERT INTO users (user_id, email, password_hash, character_name, steam_id, display_name, is_admin)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
    stmt.run(
      userId,
  ...
  getUserById: (userId) => {
    const stmt = db.prepare('SELECT user_id, email, character_name, steam_id, display_name, email_verified, created_at, last_login FROM users WHERE user_id = ?');
