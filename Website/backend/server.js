  ...
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: isDevelopment 
        ? ["'self'", "http://localhost:3000", "http://127.0.0.1:3000", "https://api.stripe.com", "https://api.sandbox.paypal.com", "https://api.paypal.com"]
        : ["'self'", "https://api.stripe.com", "https://api.sandbox.paypal.com", "https://api.paypal.com"],
...
// CSRF protection (exclude webhooks which use signature verification)
...
// Process delivery queue periodically
setInterval(async () => {
  try {
...
// Cleanup old login attempts periodically (every hour)
setInterval(async () => {
  try {
